...
public class StoreTest {
		...
		Describe("given a store definition", () -> {
			...
			Context("given the application context", () -> {
				It("should have a store bean", () -> {
					assertThat(context.getBean(TestContentRepository.class),
							is(not(nullValue())));
				});
				It("should have the core spring content service beans", () -> {
					assertThat(context.getBean(ContentStoreService.class),
							is(not(nullValue())));
					assertThat(context.getBean(RenditionService.class),
							is(not(nullValue())));
					assertThat(context.getBean(AnnotatedStoreEventInvoker.class),
							is(not(nullValue())));
				});
				...
				It("should have an TestAssociativeStore bean", () -> {
					assertThat(context.getBean(TestAssociativeStore.class),
							is(not(nullValue())));
				});
				It("should have an TestAssociativeAndContentStore bean", () -> {
					assertThat(context.getBean(TestAssociativeAndContentStore.class),
							is(not(nullValue())));
				});
			});
		});
	...
	public interface TestAssociativeAndContentStore
			extends AssociativeStore<Object, URI>, ContentStore<Object, URI> {
	}
