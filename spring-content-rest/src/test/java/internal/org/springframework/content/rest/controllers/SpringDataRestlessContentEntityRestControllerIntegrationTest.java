...
import internal.org.springframework.content.rest.TestEntity4;
...
public class SpringDataRestlessContentEntityRestControllerIntegrationTest {
	...
	@Autowired TestEntity4Repository repository;
	@Autowired TestEntity4ContentRepository contentRepository;
	...
	private TestEntity4 testEntity;
	
	{
		Describe("ContentEntityRestController", () -> {
			...
			Context("given an Entity is a ContentRepository", () -> {
				BeforeEach(() -> {
					testEntity = repository.save(new TestEntity4());
				});
				Context("a GET to /{repository}/{id} accepting a content mime-type", () -> {
					It("should return 404", () -> {
						mvc.perform(get("/testEntity4s/" + testEntity.id)
						.accept("text/plain"))
...
//				});
				Context("a PUT to /{repository}/{id} with a content body", () -> {
					It("should set the content and return 200", () -> {
						mvc.perform(put("/testEntity4s/" + testEntity.id.toString())
						.content("Hello New Spring Content World!")
						.contentType("text/plain"))
						.andExpect(status().isOk());

						TestEntity4 fetched = repository.findOne(testEntity.id);
				...
				Context("a DELETE to /{repsotiory}/{id} with a mime-type", () -> {
					It("should return 404", () -> {
						mvc.perform(delete("/testEntity4s/" + testEntity.id)
						.accept("text/plain"))
						.andExpect(status().isNotFound());
					});
				});
				Context("a POST to /{repository}/{id} with a multi-part form-data request", () -> {
					It("should set the content and return 200", () -> {
						...
						mvc.perform(fileUpload("/testEntity4s/" + testEntity.id.toString())
				...
				Context("given the Entity has content", () -> {
					...
					Context("a GET to /{repository}/{id}", () -> {
						It("should return the content and 200", () -> {
							MockHttpServletResponse response = mvc.perform(get("/testEntity4s/" + testEntity.id)
									.accept("text/plain"))
									.andExpect(status().isOk())
									.andReturn().getResponse();
					...
					Context("a PUT to /{repository}/{id}", () -> {
						It("should overwrite the content and return 200", () -> {
							mvc.perform(put("/testEntity4s/" + testEntity.id)
