		...
		Context("#getContent", () -> {
			...
			Context("given an application/word entity, accept mime types text/xml, text/html", () -> {
				BeforeEach(() -> {
					...
					entity = new TestEntity();
					entity.setMimeType("application/word");
					mimeTypes = Arrays.asList(new MediaType[] {
							MediaType.valueOf("text/xml"),
							MediaType.valueOf("text/html")
						});
					
					when(store.getRendition(eq(entity), eq("text/html"))).thenReturn(new ByteArrayInputStream(new byte[] {}));
				});
			...
			Context("given an application/word entity, accept mime types text/xml, text/*", () -> {
				BeforeEach(() -> {
					...
					when(store.getRendition(eq(entity), eq("text/*"))).thenReturn(new ByteArrayInputStream(new byte[] {}));
				});
