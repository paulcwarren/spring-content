		...
		Context("#storePath", () -> {
			...
			Context("given a content store with a StoreRestResource annotation that specifies a path",
					() -> {
						BeforeEach(() -> {
							ContentStore storeImpl = mock(
									ContentStoreWithAnotherPath.class);
							info = new ContentStoreInfoImpl(
									ContentStoreWithAnotherPath.class, TestEntity.class,
									storeImpl);
						});
						It("should return return the specified path", () -> {
...
//						assertThat(headers.getContentLength(), is(100L));
	...
	public static interface ContentStoreWithDeprecatedAnnotation
			extends ContentStore<TestEntity, UUID> {
		//
	}
	...
	@StoreRestResource
	public static interface ContentStoreWithAnnotation
			extends ContentStore<TestEntity, UUID> {
		//
	}

	@StoreRestResource(path = "some-other-path")
	public static interface ContentStoreWithAnotherPath
			extends ContentStore<TestEntity, UUID> {
		//
	}
