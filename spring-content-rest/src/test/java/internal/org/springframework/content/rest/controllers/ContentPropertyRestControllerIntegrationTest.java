			...
			Context("given an Entity with a simple content property", () -> {
				...
				Context("given that is has no content", () -> {
					Context("a GET to /{repository}/{id}/{contentProperty}", () -> {
						It("should return 404", () -> {
							mvc.perform(
									get("/files/" + testEntity2.id.toString() + "/child"))
					...
					Context("a PUT to /{repository}/{id}/{contentProperty}", () -> {
						It("should create the content", () -> {
							mvc.perform(
									put("/files/" + testEntity2.id.toString() + "/child")
											.content("Hello New Spring Content World!")
											.contentType("text/plain"))
									.andExpect(status().is2xxSuccessful());

							Optional<TestEntity2> fetched = repository2
									.findById(testEntity2.id);
							assertThat(fetched.isPresent(), is(true));
							assertThat(fetched.get().child.contentId,
									is(not(nullValue())));
				...
				Context("given that it has content", () -> {
					BeforeEach(() -> {
						...
						contentRepository2.setContent(testEntity2.child,
					...
					Context("a GET to /{repository}/{id}/{contentProperty}", () -> {
						It("should return the content", () -> {
							MockHttpServletResponse response = mvc
									.perform(get("/files/" + testEntity2.id.toString()
											+ "/child").accept("text/plain"))
					...
					Context("a DELETE to /{repository}/{id}/{contentProperty}", () -> {
						It("should delete the content", () -> {
							mvc.perform(delete(
									"/files/" + testEntity2.id.toString() + "/child"))
								...
								It("should return the original content and 200", () -> {
									MockHttpServletResponse response = mvc
											.perform(get("/files/"
													+ testEntity2.id.toString()
													+ "/child/"
													+ testEntity2.child.contentId).accept(
															new String[] { "text/xml",
																	"text/*" }))
											.andExpect(status().isOk()).andReturn()
											.getResponse();
							...
							() -> {
								It("should overwrite the content", () -> {
									mvc.perform(put("/files/"
											+ testEntity2.id.toString() + "/child/"
											+ testEntity2.child.contentId).content(
				...
				Context("given that is has no content", () -> {
					Context("a GET to /{repository}/{id}/{contentProperty}", () -> {
						It("should return 406 MethodNotAllowed", () -> {
							mvc.perform(get(
									"/files/" + testEntity2.id.toString() + "/children/"))
					...
					Context("a PUT to /{repository}/{id}/{contentProperty}", () -> {
						It("should append the content to the entity's content property collection",
								() -> {
									mvc.perform(put("/files/" + testEntity2.id.toString()
											+ "/children/").content(
									...
									assertThat(fetched.get().children.get(0).contentLen,
											is(31L));
									assertThat(fetched.get().children.get(0).mimeType,
											is("text/plain"));
								});
					});
						...
						It("should append the content to the entity's content property collection",
								() -> {
									...
									mvc.perform(fileUpload("/files/"
											+ testEntity2.id.toString() + "/children/")
									...
									assertThat(fetched.get().children.get(0).contentLen,
											is(31L));
									assertThat(fetched.get().children.get(0).fileName,
											is("test-file.txt"));
									assertThat(fetched.get().children.get(0).mimeType,
											is("text/plain"));
								});
				...
				Context("given that is has content", () -> {
					...
					Context("a DELETE to /{repository}/{id}/{contentCollectionProperty}/{contentId}",
							() -> {
								It("should delete the content", () -> {
									mvc.perform(
											delete("/files/" + testEntity2.id.toString()
													+ "/children/" + child2.contentId))
											.andExpect(status().isNoContent());
