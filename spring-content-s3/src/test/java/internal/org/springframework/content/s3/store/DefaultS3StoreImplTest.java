...
public class DefaultS3StoreImplTest {
            ...
            Describe("Store", () -> {
                Context("#getResource", () -> {
                    Context("given the store's ID is an S3ObjectId type", () ->{
                        BeforeEach(() -> {
                            s3ObjectIdBasedStore = new DefaultS3StoreImpl<ContentProperty, S3ObjectId>(loader, converter, client,
                                    S3ObjectIdResolver.createS3ObjectIdResolver(
                                            S3ObjectId::getBucket,
                                            S3ObjectId::getKey,
                                            id -> {
                                                Assert.notNull(id.getBucket(), "Bucket must not be null");
                                                Assert.notNull(id.getKey(), "Key must not be null");
                                            }
                                    )
                    ...
                    Context("given the store's ID is a custom ID type", () -> {
                        JustBeforeEach(() -> {
                            customS3ContentIdBasedStore = new DefaultS3StoreImpl<ContentProperty, CustomContentId>(loader,
                                    converter,
                                    client,
                                    resolver,
                                    defaultBucket);
                        ...
                        Context("given a default bucket is set", () -> {
                            ...
                            Context("given the resolver is created with the static constructor function", () -> {
                                BeforeEach(() -> {
                                    resolver = S3ObjectIdResolver.createS3ObjectIdResolver(
                            ...
                            Context("given the resolver is an anonymous class", () -> {
                                BeforeEach(() -> {
                                    resolver = new S3ObjectIdResolver<CustomContentId>() {
                                        @Override
                                        public String getBucket(CustomContentId idOrEntity, String defaultBucketName) {
                            ...
                            Context("given the resolver is a validating resolver", () -> {
                                BeforeEach(() -> {
                                    resolver = S3ObjectIdResolver.createS3ObjectIdResolver(
                                            CustomContentId::getCustomer,
                                            CustomContentId::getObjectId,
                                            id -> Assert.notNull(id.getCustomer(), "bad id")
                                    );
                                });
                                Context("given an invalid ID", () -> {
                                    BeforeEach(() -> {
                                        customId = new CustomContentId(/*invalid --->*/null, "some-object-id");
                                    });
                        ...
                        Context("given a default bucket is not set", () -> {
                            ...
                            Context("given a resolver that does not validate", () -> {
                                ...
                                Context("when called with an ID that doesn't specify a bucket either", () -> {
                                    ...
                                    It("should throw an error", () -> {
                                        assertThat(e, is(not(nullValue())));
                                        assertThat(e.getMessage(), is("Bucket not set"));
                                    });
                                });
                            });
                        });
                    ...
                    Context("given the default associative store id resolver", () -> {
                        ...
                        Context("given a default bucket", () -> {
                            ...
                            Context("when called with an entity that doesn't have an @Bucket value", () -> {
                                ...
                                It("should fetch the resource", () -> {
                                    verify(loader).getResource(matches("^s3://default-defaultBucket/converted/uuid"));
                                });
                        ...
                        Context("given a default bucket", () -> {
                            ...
                            Context("when called with an entity", () -> {
                                BeforeEach(() -> {
                                    ...
                                    when(converter.convert(eq("custom-object-id"), eq(String.class))).thenReturn("/custom/object/id");
                                });
                                It("should fetch the resource", () -> {
    ...
    static S3ObjectIdResolver<CustomContentId> customIdResolver = S3ObjectIdResolver.createS3ObjectIdResolver(
        CustomContentId::getCustomer,
        CustomContentId::getObjectId,
        id -> {
            Assert.notNull(id.getCustomer(), "Unable to determine defaultBucket.  Customer is null");
            Assert.notNull(id.getObjectId(), "Unable to determine key.  ObjectId is null");
        }
    );
}
