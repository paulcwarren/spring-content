...
public class ElasticsearchIndexer extends AbstractStoreEventListener<Object> {
	...
	protected void onAfterSetContent(AfterSetContentEvent event) {
		String id = BeanUtils.getFieldWithAnnotation(event.getSource(), ContentId.class)
				.toString();
		...
		try {
			...
			throw new StoreAccessException(String.format(
					"IOException error while converting stream to byte array for content ID:%s",
					id), e.getCause());
		}
		...
		request.put("original-content", content).put("contentId", id);
		...
		try {
			ResponseEntity<String> response = template.exchange(
					"http://search-spring-content-cc4bqyhqoiokxrakhfp4s2y3tm.us-east-1.es.amazonaws.com/docs/doc/"
							+ id,
			...
			throw new StoreAccessException(String.format(
					"Unexpected error attempting to index content for content id %s", id),
					rce);
		}
	}
	...
	protected void onBeforeUnsetContent(BeforeUnsetContentEvent event) {
		...
		try {
			...
			throw new StoreAccessException(String.format(
					"Unexpected error attempting to delete index for content id %s", id),
					rce);
		}
	}
	...
	protected void handleResponse(String id, HttpStatus httpStatus) {
		if (httpStatus.is5xxServerError() || httpStatus.is4xxClientError()) {
			throw new StoreAccessException(String
					.format("Indexing error while storing content for contentId %s", id));
		}
	}
}