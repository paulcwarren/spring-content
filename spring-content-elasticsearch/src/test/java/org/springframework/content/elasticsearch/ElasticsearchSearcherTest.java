		...
		Describe("ElasticsearchSearcher", () -> {
				...
				Context("given elasticsearch return a result", () -> {
					BeforeEach(() -> {
						...
						JsonObject obj = parser.parse(
								"{\"took\":3,\"timed_out\":false,\"_shards\":{\"total\":5,\"successful\":5,\"failed\":0},\"hits\":{\"total\":2,\"max_score\":0.221545,\"hits\":[{\"_index\":\"docs\",\"_type\":\"doc\",\"_id\":\"1\",\"_score\":0.221545,\"_source\":{\"original-content\":\"UWJv\",\"id\":\"12345\"}},{\"_index\":\"docs\",\"_type\":\"doc\",\"_id\":\"2\",\"_score\":0.73642,\"_source\":{\"original-content\":\"pb24u\",\"id\":\"67890\"}}]}}")
								.getAsJsonObject();
					...
					It("should return a result", () -> {
						...
						ArgumentCaptor<Action<SearchResult>> argumentCaptor = ArgumentCaptor
						...
						assertThat(
								argumentCaptor.getValue()
										.getData(new GsonBuilder().create()),
								containsString("\"query\":\"something\""));
					});
				});
				...
				Context("given elastic search is available", () -> {
					...
					It("should call the client with a correctly formed query", () -> {
						...
						String query = argumentCaptor.getValue()
								.getData(new GsonBuilder().create());
						assertThat(query,
								containsString("\"query\":\"one AND two AND three\""));
						assertThat(result, is(not(nullValue())));
						assertThat(result, hasItem("12345"));
						assertThat(result, hasItem("67890"));
					});
				...
				Context("given an elastic search", () -> {
					...
					It("should call the client with a correctly formed query", () -> {
						...
						String query = argumentCaptor.getValue()
								.getData(new GsonBuilder().create());
						assertThat(query,
								containsString("\"query\":\"one OR two OR three\""));
						assertThat(result, is(not(nullValue())));
						assertThat(result, hasItem("12345"));
						assertThat(result, hasItem("67890"));
					});
				...
				Context("given an elastic search", () -> {
					...
					It("should call the client with a correctly formed query", () -> {
						...
						String query = argumentCaptor.getValue()
								.getData(new GsonBuilder().create());
						assertThat(query, is(
								"{\"query\":{\"query_string\":{\"query\":\"\\\"one two three\\\"~10\"}}}"));
						assertThat(result, is(not(nullValue())));
						assertThat(result, hasItem("12345"));
						assertThat(result, hasItem("67890"));
					});
				...
				Context("given an elastic search", () -> {
					...
					It("should call the client with a correctly formed query", () -> {
						...
						String query = argumentCaptor.getValue()
								.getData(new GsonBuilder().create());
						assertThat(query, is(
								"{\"query\":{\"query_string\":{\"query\":\"one^2.0 two^3.0\"}}}"));
						assertThat(result, is(not(nullValue())));
						assertThat(result, hasItem("12345"));
						assertThat(result, hasItem("67890"));
					});
			...
			Context("#findKeywordsStartsWithAndEndsWith", () -> {
				JustBeforeEach(() -> {
					...
					String endsWith = "two";
					try {
						result = searcher.findKeywordStartsWithAndEndsWith(startsWith,
								endsWith);
					}
					catch (Exception e) {
						exception = e;
				...
				Context("given an elastic search", () -> {
					...
					It("should call the client with a correctly formed query", () -> {
						...
						String query = argumentCaptor.getValue()
								.getData(new GsonBuilder().create());
						assertThat(query, is(
								"{\"query\":{\"query_string\":{\"query\":\"one*two\"}}}"));
						assertThat(result, is(not(nullValue())));
						assertThat(result, hasItem("12345"));
						assertThat(result, hasItem("67890"));
					});
