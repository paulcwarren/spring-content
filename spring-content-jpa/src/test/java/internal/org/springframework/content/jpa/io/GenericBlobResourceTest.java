		...
		Describe("GenericBlobResource", () -> {
			...
			Context("#getInputStream", () -> {
				...
				Context("given the blob exists in the database", () -> {
					BeforeEach(() -> {
						when(rs.next()).thenReturn(true);
						when(rs.getBinaryStream(1)).thenReturn(
								new ByteArrayInputStream("Hello Spring Content PostgreSQL BLOBby world!".getBytes()));
					});
					...
					It("should return the correct content", () -> {
						...
						try {
							expected = new ByteArrayInputStream(
									"Hello Spring Content PostgreSQL BLOBby world!".getBytes());
			...
			Context("#getOutputStream", () -> {
				...
				Context("given the blob exists in the database", () -> {
					...
					It("should use update to overwrite the content", () -> {
						verify(conn, timeout(100)).prepareStatement(
								org.mockito.hamcrest.MockitoHamcrest.argThat(containsString("UPDATE BLOBS")));

						verify(preparedStatement, timeout(100)).setBinaryStream(eq(1),
								org.mockito.hamcrest.MockitoHamcrest.argThat(is(instanceOf(InputStream.class))));
						verify(preparedStatement, timeout(100)).setInt(2, 9999);
				...
				Context("given the blob does not exist in the database", () -> {
					...
					It("should use insert to add the content", () -> {
						verify(conn, timeout(100)).prepareStatement(
								org.mockito.hamcrest.MockitoHamcrest.argThat(containsString("INSERT INTO BLOBS")),
