...

	{
		Describe("Searchable", () -> {
			...
			Context("#findKeyword", () -> {
				Context("given a keyword", () -> {
					...
					JustBeforeEach(() -> {
						search = new SolrSearchService(solr, solrProperties,
								Document.class);
						try {
					...
					It("should execute a query", () -> {
						ArgumentCaptor<SolrRequest> argument = forClass(
								SolrRequest.class);
						verify(solr).request(argument.capture(), anyObject());
						assertThat(argument.getValue().getParams().get("q"),
								is("(" + keyword + ") AND id:"
					...
					Context("given a SolrServerException from solr", () -> {
						BeforeEach(() -> {
							when(solr.request(anyObject(), anyObject()))
			...
			Context("#findAnyKeywords", () -> {
				Context("given an array of terms", () -> {
					...
					It("should execute a query", () -> {
						...
						verify(solr).request(argument.capture(), anyObject());
						assertThat(argument.getValue().getParams().get("q"),
								is("(something OR else OR bobbins) AND id:"
										+ Document.class.getCanonicalName() + "\\:*"));
						assertThat(argument.getValue().getParams().get("fl"), is("id"));
					});
					It("should map results to set of IDs", () -> {
				...
				Context("given a starting and ending keyword", () -> {
					...
					JustBeforeEach(() -> {
						search = new SolrSearchService(solr, solrProperties,
								Document.class);
						try {
							result = search.findKeywordStartsWithAndEndsWith(starts,
									ends);
						}
						catch (Exception e) {
							this.e = e;
						...
						() -> {
							...
							JustBeforeEach(() -> {
								search = new SolrSearchService(solr, solrProperties,
										Document.class);
								try {
									result = search.findAllKeywordsWithWeights(terms,
											weights);
								}
								catch (Exception e) {
									this.e = e;
							...
							It("should execute a query", () -> {
								...
								verify(solr).request(argument.capture(), anyObject());
								assertThat(argument.getValue().getParams().get("q"),
										is("((foo)^1.59 AND (bar)^200.0) AND id:"
												+ Document.class.getCanonicalName()
												+ "\\:*"));
								assertThat(argument.getValue().getParams().get("fl"),
										is("id"));
							});
							It("should map results to set of IDs", () -> {
								assertThat(result.size(), is(1));
			...
			Context("#parseTerms", () -> {
				Context("given multiple terms", () -> {
					...
					Context("given AND", () -> {
						...
						JustBeforeEach(() -> {
							search = new SolrSearchService(null, solrProperties,
									Document.class);
						});
		...
		Context("#executeQuery", () -> {
			...
			Context("When a username/password is specified", () -> {
				...
				JustBeforeEach(() -> {
					search = spy(
							new SolrSearchService(solr, solrProperties, Document.class));
