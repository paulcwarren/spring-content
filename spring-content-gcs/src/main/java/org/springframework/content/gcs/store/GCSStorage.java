...
public class GCSStorage {
	...
	/**
	 * Creating a blob with no content.
	 */
	...
	/**
	 * Creating a blob from a byte array.
	 */
	public Blob createBlobFromByteArray(String bucketName, String blobName, byte[] fileBytes) {
		BlobId blobId = BlobId.of(bucketName, blobName);
	...
	/*public Blob createBlobFromByteArray(String bucketName, MultipartFile file) throws IOException {
	...
	/*public Blob createBlobFromByteArray(String bucketName, Document document) throws IOException {

		BlobId blobId = BlobId.of(bucketName, document.getOriginalFileName());
		// BlobInfo blobInfo = BlobInfo.newBuilder(blobId).setAcl(new
		// ArrayList(Arrays.asList(Acl.of(User.ofAllUsers(),
		// com.google.cloud.storage.Acl.Role.READER)))).setContentType(document.getContentType()).build();
		BlobInfo blobInfo = BlobInfo.newBuilder(blobId).setContentType(document.getContentType()).build();

		Blob blob = storage.create(blobInfo, document.getContent());
	...
	/**
	 * Example of getting information on a blob, only if its metageneration
	 * matches a value, otherwise a {@link StorageException} is thrown.
	 */
	...
	/**
	 * Example of udating a blob, only if the blob's metageneration matches a
	...
	/**
	 * Example of deleting a blob, only if its generation matches a value,
	 * otherwise a {@link StorageException} is thrown.
	 */
	...
	public Blob batch(String bucketName, String blobName1, String blobName2) {
		...
		batch.submit();
		Blob blob = result.get(); // returns get result or throws
									// StorageException
		// [END batch]
		return blob;
	}
	...
	/**
	 * Example of creating a signed URL that is valid for 2 weeks, using the
	 * default credentials for signing the URL.
	 */
	...
	/**
	 * Example of getting information on several blobs using a single batch
	 * request.
	 */
	// [TARGET get(BlobId...)]
	// [VARIABLE "my_unique_bucket"]
	...
	public Bucket disableRequesterPays(String bucketName) {
		...
		// The name of the bucket to disable requester-paying for, e.g.
		// "my-bucket"
		// String bucketName = "my-bucket"
	...
	public Bucket getRequesterPaysStatus(String bucketName) throws StorageException {
		...
		// The name of the bucket to retrieve requester-pays status, eg.
		// "my-bucket"
		// String bucketName = "my-bucket"
	...
	/**
	 * Downloading a file using Requester pay.
	 */
